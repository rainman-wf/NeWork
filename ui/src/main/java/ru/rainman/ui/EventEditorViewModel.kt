...
class EventEditorViewModel @Inject constructor(
    ...
    val eventStatus = SingleLiveEvent<Status>()
    ...
    fun publish(context: Context) {
        ...
        viewModelScope.launch {
            eventStatus.postValue(Status.Loading)
            try {
                eventRepository.create(newEventDto)
                eventStatus.postValue(Status.Success)
            } catch (e: Exception) {
    ...
    private fun setError(message: String): Boolean {
        eventStatus.postValue(Status.Error(message))
        return true
    }
    ...
    fun setContent(content: String) {
        _editableEvent.postValue(_editableEvent.value!!.copy(content = content))
    }
}