...
class SelectUsersDialogViewModel @Inject constructor(
    ...
    private val _filterState = MutableLiveData(UsersFilter(false, null))
    ...
    init {
        _selectableUsers.addSource(_filterState) { filter ->
            _selectableUsers.postValue(combineSelectable(_users.value ?: emptyList(), _selectedIds.value ?: emptySet()).filter {
                filter.selected?.let { selected ->  it.selected == selected } ?: true &&
                        filter.name?.let { name -> it.user.name.lowercase().contains(name.lowercase()) } ?: true
            })
        }
